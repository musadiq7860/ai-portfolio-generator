...
const Onboarding = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.append('job_target', jobTarget || 'Software Engineering roles');
      formData.append('skills_to_emphasize', skillsToEmphasize || 'Full stack development and problem solving');
      formData.append('one_liner', oneLiner || 'Dedicated software professional');
      ...
      formData.append('full_name', user.user_metadata?.full_name || 'Developer');
      ...
      if (!res.ok || data.error) {
        const errorMsg = data.error || data.detail || 'Generation failed.';
        const stringifiedMsg = typeof errorMsg === 'string' ? errorMsg : JSON.stringify(errorMsg);
        console.error("DEBUG: Generation failed with:", stringifiedMsg);
        throw new Error(stringifiedMsg);
      }
